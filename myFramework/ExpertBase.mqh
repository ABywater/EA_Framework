...
public:	// Functions
	...
	////New
	virtual ENUM_NWN_SIGNAL_DIRECTION	GetCurrentSignal(CSignalBase* &signals[],
																	ENUM_NWN_SIGNAL_TYPE signalType);
...
bool		CExpertBase::LoopMain(bool newBar,bool firstTime) {
	...
	//
	////Changed
	ENUM_NWN_SIGNAL_DIRECTION	entrySignal	=	GetCurrentSignal(mEntrySignals, NWN_ENTRY_SIGNAL);
	ENUM_NWN_SIGNAL_DIRECTION	exitSignal	=	GetCurrentSignal(mExitSignals, NWN_EXIT_SIGNAL);
	...
	//
	////Changed
	if (exitSignal==NWN_SIGNAL_BOTH) {
		Trade.PositionCloseByType(mSymbol, POSITION_TYPE_BUY);
		Trade.PositionCloseByType(mSymbol, POSITION_TYPE_SELL);
	} else
	if (exitSignal==NWN_SIGNAL_BUY) {
		Trade.PositionCloseByType(mSymbol, POSITION_TYPE_BUY);
	} else
	if (exitSignal==NWN_SIGNAL_SELL) {
		Trade.PositionCloseByType(mSymbol, POSITION_TYPE_SELL);
	}
	////if (mExitSignal!=NULL) {
	////	if (mExitSignal.ExitSignal()==NWN_SIGNAL_BOTH) {
	////		Trade.PositionCloseByType(mSymbol, POSITION_TYPE_BUY);
	////		Trade.PositionCloseByType(mSymbol, POSITION_TYPE_SELL);
	////	} else
	////	if (mExitSignal.ExitSignal()==NWN_SIGNAL_BUY) {
	////		Trade.PositionCloseByType(mSymbol, POSITION_TYPE_BUY);
	////	} else
	////	if (mExitSignal.ExitSignal()==NWN_SIGNAL_SELL) {
	////		Trade.PositionCloseByType(mSymbol, POSITION_TYPE_SELL);
	////	}
	////}
	...
	////Changed
	if (entrySignal==NWN_SIGNAL_BOTH) {
	...
	if (entrySignal==NWN_SIGNAL_BUY) {

		GetMarketPrices(ORDER_TYPE_BUY, request);
		Trade.Buy(mVolume, mSymbol, request.price, request.sl, request.tp);

	} else
	if (entrySignal==NWN_SIGNAL_SELL) {
...
////		if (mEntrySignal.EntrySignal()==NWN_SIGNAL_BOTH) {
////
////			GetMarketPrices(ORDER_TYPE_BUY, request);
////			Trade.Buy(mVolume, mSymbol, request.price, request.sl, request.tp);
////
...
////
////		} else
////		if (mEntrySignal.EntrySignal()==NWN_SIGNAL_BUY) {
////
////			GetMarketPrices(ORDER_TYPE_BUY, request);
////			Trade.Buy(mVolume, mSymbol, request.price, request.sl, request.tp);
////
////		} else
////		if (mEntrySignal.EntrySignal()==NWN_SIGNAL_SELL) {
////
////			GetMarketPrices(ORDER_TYPE_SELL, request);
////			Trade.Sell(mVolume, mSymbol, request.price, request.sl, request.tp);
////
...
////New
ENUM_NWN_SIGNAL_DIRECTION	CExpertBase::GetCurrentSignal(CSignalBase* &signals[],
																		ENUM_NWN_SIGNAL_TYPE signalType) {

	ENUM_NWN_SIGNAL_DIRECTION	result	=	NWN_SIGNAL_NONE;
	ENUM_NWN_SIGNAL_DIRECTION	r2			=	NWN_SIGNAL_NONE;	//	Just working value
	int	index	=	ArraySize(signals);
	...
	if (index<=0) {
		...
		for (int i = 1; i<index; i++) {
			...
			//if (result==NWN_SIGNAL_NONE) return(result);
			...
			//	Any		+	Both	=	Any
			//	!Both		+	!Same	=	None
			if (result==NWN_SIGNAL_BOTH)		{	result	=	r2;	}
			else if (r2==NWN_SIGNAL_BOTH)		{	}
			else if (result!=r2)					{	result	=	NWN_SIGNAL_NONE;	}
